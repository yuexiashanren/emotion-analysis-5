...
f_neu = open('neutral.csv', 'w', encoding='utf-8')
f_pos = open('pos.csv', 'w', encoding='utf-8')

#课前预习筛选
for i in range(nrows1):
    #获取情感倾向列的标签值(行列序号从0开始)
	choose1 = sheet1.cell_value(i,5)
	if(choose1 == 1):
		sum += 1
		pos_sum += 1
		text_str = sheet1.cell_value(i,4)
...
for i in range(nrows2):
	...
	choose2 = sheet2.cell_value(i,5)
	if(choose2 == 1):
		sum += 1
		pos_sum += 1
		text_str = sheet2.cell_value(i,4)
		f_pos.write(str(text_str))
		f_pos.write('\n')
	elif(choose2 == -1):
		sum += 1
...
for i in range(nrows3):
	...
	if(choose3 == 1):
		sum += 1
		pos_sum += 1
		text_str = sheet3.cell_value(i,3)
		f_pos.write(str(text_str))
		f_pos.write('\n')
	elif(choose3 == -1):
		sum += 1
...
for i in range(nrows4):
    #获取情感倾向列的标签值(行列序号从1开始)
	choose4 = sheet4.cell_value(i,5)
	if(choose4 == 1):
		sum += 1
		pos_sum += 1
		text_str = sheet4.cell_value(i,4)
		f_pos.write(str(text_str))
		f_pos.write('\n')
	elif(choose4 == -1):
		sum += 1
...
f_neg.close()
print("累计语料",sum)
print("累计pos语料",pos_sum)
print("累计neu语料",neu_sum)
print("累计neg语料",neg_sum)
